...
def test_hangar_load(page: Page):
    ...
    page.goto("/swse/")
    page.wait_for_selector("#q-app", state="visible", timeout=30000)

    if page.is_visible("#hangar-dialog-card"):
        pass
    elif page.is_visible("text=No Ship Loaded"):
        page.click("button:has-text('Open Hangar')")
    else:
        page.click("#tour-hangar-btn")

    page.wait_for_selector("#hangar-dialog-card")
...
def test_custom_ship(page: Page):
    ...
    page.wait_for_selector("text=Library Manager")
    ...
    page.wait_for_selector("text=Create Custom Ship")
